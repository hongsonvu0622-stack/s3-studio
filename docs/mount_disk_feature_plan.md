...
> **Trạng thái:** Đã phân tích & đánh giá khả thi (`Feasible & Approved for Future Development`).  
> **Lựa chọn kiến trúc:** Sử dụng **Rclone Mount Engine (`rclone mount`)** kết hợp FUSE Driver (`macFUSE` / `FUSE-T` trên macOS, `WinFsp` trên Windows).
...
- **Rclone Binary**:
  ...
  - *Tùy chọn tương lai*: Nếu chưa có, ứng dụng có thể bấm nút tự tải binary `rclone` nhúng trực tiếp vào ứng dụng.
...
2. **Hộp thoại Quản lý Ổ đĩa ảo (`src/components/DiskMountModal.jsx`)**:
   - **Thẻ Kiểm tra Hệ thống (System Status Banner)**:
     ...
     - Nếu thiếu, hiển thị cảnh báo rõ ràng kèm hướng dẫn cài đặt nhanh.
...
## 4. Các Vấn đề cần Lưu ý & Đánh giá Rủi ro (Risk Assessment)
